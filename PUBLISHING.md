...
3. Choose a Cloady-owned GPG signing key, or have a release maintainer create one using [HashiCorp's signing-key instructions](https://developer.hashicorp.com/terraform/registry/providers/publishing#preparing-and-adding-a-signing-key). Retain the key and its recovery material securely; use the same key for future releases.
